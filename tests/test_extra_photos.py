...
def test_photo_avec_aspect_et_numero(repo):
    ...
    assert noms(repo.gq.extra_photos("chene")) == ["chene-2.jpg", "chene-feuille-1.jpg"]
...
def test_stem_nu_accepte(repo):
    ...
    assert noms(repo.gq.extra_photos("chene")) == ["chene.jpg"]
...
def test_un_stem_ne_prend_pas_les_photos_de_celui_qu_il_prefixe(repo):
    ...
    assert noms(repo.gq.extra_photos("ail")) == ["ail-1.jpg"]
...
def test_l_espece_prefixee_garde_toutes_ses_photos(repo):
    ...
    assert noms(repo.gq.extra_photos("ail_des_ours")) == ["ail_des_ours-1.jpg",
...
def test_ignore_les_fichiers_non_images(repo):
    ...
    assert noms(repo.gq.extra_photos("chene")) == ["chene-feuille-1.jpg"]
...
def test_accepte_jpeg_et_png(repo):
    ...
    assert noms(repo.gq.extra_photos("chene")) == ["chene-1.jpeg", "chene-2.PNG"]
...
def test_dossier_quiz_extra_absent(repo):
    ...
    assert repo.gq.extra_photos("chene") == []
...
def test_le_parseur_n_attribue_que_les_bonnes_photos(repo):
    ...
    repo.atlas("Test.md", [ligne("ail.jpg", "Ail", "Allium sativum"),
